// shared big endian read/write helpers for the wire codecs
...
// bounds-checked span variants; return 0 when the read would run off the end
...
inline void append_be_u32(std::vector<std::byte>& out, std::uint32_t v) {
...
inline void append_be_u64(std::vector<std::byte>& out, std::uint64_t v) {
    for (int i = 7; i >= 0; --i) {
        out.push_back(static_cast<std::byte>((v >> (i * 8)) & 0xff));
    }
}

inline void append_be_u32(QByteArray& out, std::uint32_t v) {
...
inline void append_be_u64(QByteArray& out, std::uint64_t v) {
    for (int i = 7; i >= 0; --i) {
        out.append(static_cast<char>((v >> (i * 8)) & 0xff));
    }
}

inline std::uint32_t read_be_u32(const QByteArray& b, int offset) {
    if (offset < 0 || offset + 4 > b.size()) return 0;
    return read_be_u32(reinterpret_cast<const std::byte*>(b.constData() + offset));
}
